...
def run_tidy(args, clang_tidy_binary, tmpdir, build_path, queue, lock, failed_files):
    ...
    while True:
        ...
        with lock:
            full_output = (err or b'') + b'\n' + output
            for line in full_output.decode("utf-8").split("\n"):
                if ("warnings generated." in line and (len(line) - len("warnings generated.")) <= 8) or line in ["", " "]:
                    continue
                if "no checks enabled" in line:
                    has_failed = False
                    break
...
def main():
    ...
    try:
        ...
        for name in files:
            is_dep = '_deps' in name
            is_vendor_code = 'vendor' in name
            if file_name_re.search(name) and not is_dep and not is_vendor_code:
                task_queue.put(name)
